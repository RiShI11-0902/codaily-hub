...
const InterviewInterface = () => {
  ...
  const stopRecording = () => {
    ...
    toast({
      ...
      description: "You can now edit or save your answer.",
    });
  };
  ...
  const saveAnswer = () => {
    if (!transcript.trim() && !answerText.trim()) {
      toast({
        title: "No answer provided",
        description:
          "Please record your answer or write a text response before saving.",
        variant: "destructive",
      });
      return;
    }
  ...
  const submitAllAnswers = async (answers: RecordedAnswer[]) => {
    ...
    try {
      ...
      toast({
        title: "Interview completed!",
        description:
          "Your answers have been submitted successfully. Feedback will be shown shortly.",
      });
    } catch (error) {
      console.error("Error submitting interview:", error);
      toast({
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <QuestionCard question={currentQuestion} currentIndex={currentQuestionIndex}>
        <AnswerTextarea
          value={answerText || transcript} // live transcript shown
          onChange={setAnswerText}
          disabled={isRecording}
        />
        ...
        {listening && (
          <p className="text-sm text-muted-foreground mt-2">
            Listening... (Live transcript updating)
          </p>
        )}
      </QuestionCard>
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold">
              {recordedAnswers.length} / {questions.length}
            </span>
          </div>
        </CardContent>
      </Card>
